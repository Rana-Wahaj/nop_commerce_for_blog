...
public partial class ExportManager : IExportManager
{
    ...
    protected readonly IWahajCategoryService _wahajcategoryService;
    ...
    public ExportManager(AddressSettings addressSettings,
        ...
        IWahajCategoryService wahajcategoryService,
        ...
        ProductEditorSettings productEditorSettings)
    {
        ...
        _wahajcategoryService = wahajcategoryService;
    ...
    public virtual async Task<byte[]> ExportCategoriesToXlsxAsync(IList<CategoryWahaj> categories)
    {
        var parentCategories = new List<CategoryWahaj>();
        if (_catalogSettings.ExportImportCategoriesUsingCategoryName)
            ...
            parentCategories.AddRange(await _wahajcategoryService.GetCategoriesByIdsAsync(categories.Select(c => c.ParentCategoryId).Where(id => id != 0).ToArray()));

        var languages = await _languageService.GetAllLanguagesAsync(showHidden: true);

        var localizedProperties = new[]
        {
            new PropertyByName<CategoryWahaj, Language>("Id", (p, l) => p.Id),
            new PropertyByName<CategoryWahaj, Language>("Name", async (p, l) => await _localizationService.GetLocalizedAsync(p, x => x.Name, l.Id, false)),
            new PropertyByName<CategoryWahaj, Language>("MetaKeywords", async (p, l) => await _localizationService.GetLocalizedAsync(p, x => x.MetaKeywords, l.Id, false)),
            new PropertyByName<CategoryWahaj, Language>("MetaDescription", async (p, l) => await _localizationService.GetLocalizedAsync(p, x => x.MetaDescription, l.Id, false)),
            new PropertyByName<CategoryWahaj, Language>("MetaTitle", async (p, l) => await _localizationService.GetLocalizedAsync(p, x => x.MetaTitle, l.Id, false)),
            new PropertyByName<CategoryWahaj, Language>("SeName", async (p, l) => await _urlRecordService.GetSeNameAsync(p, l.Id, returnDefaultValue: false), await IgnoreExportCategoryPropertyAsync())
        };

        //property manager 
        var manager = new PropertyManager<CategoryWahaj, Language>(new[]
        {
            new PropertyByName<CategoryWahaj, Language>("Id", (p, l) => p.Id),
            new PropertyByName<CategoryWahaj, Language>("Name", (p, l) => p.Name),
            new PropertyByName<CategoryWahaj, Language>("Description", (p, l) => p.Description),
            new PropertyByName<CategoryWahaj, Language>("CategoryTemplateId", (p, l) => p.CategoryTemplateId),
            new PropertyByName<CategoryWahaj, Language>("MetaKeywords", (p, l) => p.MetaKeywords, await IgnoreExportCategoryPropertyAsync()),
            new PropertyByName<CategoryWahaj, Language>("MetaDescription", (p, l) => p.MetaDescription, await IgnoreExportCategoryPropertyAsync()),
            new PropertyByName<CategoryWahaj, Language>("MetaTitle", (p, l) => p.MetaTitle, await IgnoreExportCategoryPropertyAsync()),
            new PropertyByName<CategoryWahaj, Language>("SeName", async (p, l) => await _urlRecordService.GetSeNameAsync(p, 0), await IgnoreExportCategoryPropertyAsync()),
            new PropertyByName<CategoryWahaj, Language>("ParentCategoryId", (p, l) => p.ParentCategoryId),
            new PropertyByName<CategoryWahaj, Language>("ParentCategoryName", async (p, l) =>
            {
                var category = parentCategories.FirstOrDefault(c => c.Id == p.ParentCategoryId);
                return category != null ? await _wahajcategoryService.GetFormattedBreadCrumbAsync(category) : null;

            }, !_catalogSettings.ExportImportCategoriesUsingCategoryName),
            new PropertyByName<CategoryWahaj, Language>("Picture", async (p, l) => await GetPicturesAsync(p.PictureId)),
            new PropertyByName<CategoryWahaj, Language>("PageSize", (p, l) => p.PageSize, await IgnoreExportCategoryPropertyAsync()),
            new PropertyByName<CategoryWahaj, Language>("PriceRangeFiltering", (p, l) => p.PriceRangeFiltering, await IgnoreExportCategoryPropertyAsync()),
            new PropertyByName<CategoryWahaj, Language>("PriceFrom", (p, l) => p.PriceFrom, await IgnoreExportCategoryPropertyAsync()),
            new PropertyByName<CategoryWahaj, Language>("PriceTo", (p, l) => p.PriceTo, await IgnoreExportCategoryPropertyAsync()),
            new PropertyByName<CategoryWahaj, Language>("ManuallyPriceRange", (p, l) => p.ManuallyPriceRange, await IgnoreExportCategoryPropertyAsync()),
            new PropertyByName<CategoryWahaj, Language>("AllowCustomersToSelectPageSize", (p, l) => p.AllowCustomersToSelectPageSize, await IgnoreExportCategoryPropertyAsync()),
            new PropertyByName<CategoryWahaj, Language>("PageSizeOptions", (p, l) => p.PageSizeOptions, await IgnoreExportCategoryPropertyAsync()),
            new PropertyByName<CategoryWahaj, Language>("ShowOnHomepage", (p, l) => p.ShowOnHomepage, await IgnoreExportCategoryPropertyAsync()),
            new PropertyByName<CategoryWahaj, Language>("IncludeInTopMenu", (p, l) => p.IncludeInTopMenu, await IgnoreExportCategoryPropertyAsync()),
            new PropertyByName<CategoryWahaj, Language>("Published", (p, l) => p.Published, await IgnoreExportCategoryPropertyAsync()),
            new PropertyByName<CategoryWahaj, Language>("DisplayOrder", (p, l) => p.DisplayOrder)
        }, _catalogSettings, localizedProperties, languages);
