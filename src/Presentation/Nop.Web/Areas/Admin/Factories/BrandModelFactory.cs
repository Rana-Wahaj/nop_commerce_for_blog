...
public partial class BrandModelFactory : IBrandModelFactory
{
    ...
    public virtual async Task<BrandListModel> PrepareBrandListModelAsync(BrandSearchModel searchModel)
    {
        ...
        //get manufacturers
        var manufacturers = await _brandService.GetAllBrandsAsync(showHidden: true,
        ...
        //prepare grid model
        var model = await new BrandListModel().PrepareToGridAsync(searchModel, manufacturers, () =>
        {
            //fill in model values from the entity
            return manufacturers.SelectAwait(async manufacturer =>
            {
                var brandModel = manufacturer.ToModel<BrandModel>();

                brandModel.SeName = await _urlRecordService.GetSeNameAsync(manufacturer, 0, true, false);
